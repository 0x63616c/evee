...
| ai | latest | Vercel AI SDK — streamText, tool calling |
| @openrouter/ai-sdk-provider | latest | OpenRouter LLM provider for AI SDK |
...
| assistant-ui | latest | AI chat UI components (React) |
...
| Migrations | drizzle-kit | ^0.31 | `bunx drizzle-kit migrate` |
...
|---------|---------|--------|
| OpenRouter | LLM API gateway (OpenAI-compatible) | `OPENROUTER_API_KEY` in `apps/api/.env` |
...
| OPENROUTER_API_KEY | api | Yes | OpenRouter API key |
| PORT | api | No | API port (default: 4201) |
